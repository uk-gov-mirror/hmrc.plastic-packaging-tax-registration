...
import util.RetryISpec.{neverRetry, noParticularReason, retryFailures, someRetries}
...
class RetryISpec extends AnyWordSpec with Matchers with DefaultAwaitTimeout {
  ...
  "A retryable" should {
    ...
    "not retry" when {
      "configured as such" in {
        ...
        intercept[IllegalStateException] {
          await(
            retryable.retry()(retryFailures, noParticularReason)(
    ...
    "delay before retrying" in {
      ...
      intercept[IllegalStateException] {
        await(
          retryable.retry(FiniteDuration(retryDelayMs, TimeUnit.MILLISECONDS))(retryFailures,
                                                                               noParticularReason
          )(consistentlyFailingOperation.doIt())
        )
      }
