...
class SubscriptionServiceSpec
    extends AnyWordSpec with RegistrationBuilder with must.Matchers with NrsTestData {
    ...
    "notify NRS" in new Fixture {
      when(
        ...
        Future.successful(
          SubscriptionSuccessfulResponse(
            "PPT_REF",
            ZonedDateTime.of(2022, 12, 14, 10, 32, 12, 765, ZoneId.of("UTC")),
            "FORM_BUNDLE_NO"
          )
        )
      )
      ...
      val jsonStr: String =
        new String(Base64.getDecoder.decode(payloadCaptor.getValue), StandardCharsets.UTF_8)
      ...
      metadata.userSubmissionTimestamp mustBe ZonedDateTime.of(2022,
                                                               12,
                                                               14,
                                                               10,
                                                               32,
    ...
    "return the success information" in new Fixture {
      when(
        ...
        Future.successful(
          SubscriptionSuccessfulResponse(
            "PPT_REF_4",
            ZonedDateTime.of(2022, 12, 22, 10, 32, 12, 765, ZoneId.of("UTC")),
            "FORM_BUNDLE_NO_4"
          )
        )
      )
